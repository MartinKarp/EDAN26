...
case class ChangePred(in: BitSet, v: Vertex);
case class Finish();
...
class Controller(val cfg: Array[Vertex]) extends Actor {
  ...
  def act() {
    react {
      ...
      case ChangePred(in: BitSet, v: Vertex) => {
          v ! new Change(in)
          act();
      }
      case Work() => {
      ...
      case Finish() => {
          ...
          println(0.001*(System.currentTimeMillis() - begin));
      }
    }
  }
}
